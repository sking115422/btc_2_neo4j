...
for i in fList:
    ...
    while f.tell() != fSize:
        ...
        for k in range(txCount):
            ...
            transactionList.append(transaction) # Append transaction dictionary to list
            thisBlock['tx'].append(tmpHex) # Append tx ID to block data
    ...
    f.close()
    with open(dirB_tx + 'trlist_' + blockFileNum + '.json','wt', encoding='ascii') as file:
        json.dump(transactionList, file, indent=4)
    with open(dirB_blk + 'blocklist_' + blockFileNum + '.json','wt', encoding='ascii') as file:
